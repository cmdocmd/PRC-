...
private:
    ...
    {
        ar &username, &password, &email, &mac, &country, &currentWorld, &name, &guild, &tradingme, &itemlist,
            &netID, &userID, &back, &hand, &feet, &face, &hair, &shirt, &pants, &ances, &neck, &mask, &cpx, &cpy, &x, &y, &level,
            &xp, &to_break, &invSize, &adminLevel, &effect, &dropid, &gem, &ban, &bantime, &mute, &mutetime, &curse, &cursetime,
            &wrenchx, &wrenchy, &skinColor, &inAccount, &InLobby, &usingDialog, &isCorrect, &isRotatedLeft, &joinClothesUpdated,
            ...
            &havePineappleFloag, &haveFlyingPineapple, &haveSuperSupporterName, &haveSupperPineapple, &usingDoor, &inv, &trade, &friends, &worlds;
    }
...
public:
    ...
    int netID = 0, userID = 0, back = 0, hand = 0, feet = 0, face = 0, hair = 0, shirt = 0,
        ...
        xp = 0, to_break = 0, invSize = 100, adminLevel = 0, effect = 0, dropid = 0, gem = 0, ban = 0,
        bantime = 0, mute = 0, mutetime = 0, curse = 0, cursetime = 0, wrenchx = 0, wrenchy = 0, skinColor = 0x8295C3FF;
    ...
    bool inAccount = false, InLobby = false, usingDialog = false, isCorrect = false, isRotatedLeft = false, joinClothesUpdated = false,
         ...
         haveSuperSupporterName = true, haveSupperPineapple = false, usingDoor = false;
