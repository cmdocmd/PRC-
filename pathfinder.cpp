...
double calculateHValue(int x, int y, Pair dest)
{
    ...
    return ((double)sqrt(
        (x - dest.first) * (x - dest.first) + (y - dest.second) * (y - dest.second)));
}
...
bool aStarSearch(Worlds world, Pair src, Pair dest)
{
    ...
    if (isUnBlocked(world, src.first, src.second) == false || isUnBlocked(world, dest.first, dest.second) == false)
    {
    ...
    while (!openList.empty())
    {
        ...
        if (isValid(i - 1, j) == true)
        {
            ...
            else if (closedList[i - 1][j] == false && isUnBlocked(world, i - 1, j) == true)
            {
        ...
        if (isValid(i, j + 1) == true)
        {
            ...
            // list or if it is blocked, then ignore it.
            // Else do the following
            else if (closedList[i][j + 1] == false && isUnBlocked(world, i, j + 1) == true)
            {
                gNew = cellDetails[i][j].g + 1.0;
        ...
        if (isValid(i - 1, j + 1) == true)
        {
            ...
            // list or if it is blocked, then ignore it.
            // Else do the following
            else if (closedList[i - 1][j + 1] == false && isUnBlocked(world, i - 1, j + 1) == true)
            {
